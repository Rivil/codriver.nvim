...
function M.register_all()
  ...
  M.register(require("codriver.vendor.claudecode.tools.open_file"))
  M.register(require("codriver.vendor.claudecode.tools.get_current_selection"))
  M.register(require("codriver.vendor.claudecode.tools.get_open_editors"))
  M.register(require("codriver.vendor.claudecode.tools.open_diff"))
  M.register(require("codriver.vendor.claudecode.tools.get_latest_selection"))
  M.register(require("codriver.vendor.claudecode.tools.close_all_diff_tabs"))
  M.register(require("codriver.vendor.claudecode.tools.get_diagnostics"))
  M.register(require("codriver.vendor.claudecode.tools.get_workspace_folders"))
  M.register(require("codriver.vendor.claudecode.tools.check_document_dirty"))
  M.register(require("codriver.vendor.claudecode.tools.save_document"))

  -- Register internal tools without schemas (not exposed via MCP)
  M.register(require("codriver.vendor.claudecode.tools.close_tab"))
end
...
function M.handle_invoke(client, params) -- client needed for blocking tools
  ...
  if tool_data.requires_coroutine then
    ...
    require("codriver.vendor.claudecode.logger").debug("tools", "Wrapping " .. tool_name .. " in coroutine for blocking behavior")
    local co = coroutine.create(function()
      return tool_data.handler(input, client)
    end)

    require("codriver.vendor.claudecode.logger").debug("tools", "About to resume coroutine for " .. tool_name)
    local success, result = coroutine.resume(co)
    require("codriver.vendor.claudecode.logger").debug(
      "tools",
    ...
    if coroutine.status(co) == "suspended" then
      require("codriver.vendor.claudecode.logger").debug("tools", "Coroutine is suspended - tool is blocking, will respond later")
