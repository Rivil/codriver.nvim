...
local claudecode_main = require("codriver.vendor.claudecode") -- Added for version access
local logger = require("codriver.vendor.claudecode.logger")
local tcp_server = require("codriver.vendor.claudecode.server.tcp")
local tools = require("codriver.vendor.claudecode.tools.init") -- Added: Require the tools module
...
function M.start(config, auth_token)
  ...
  local callbacks = {
    ...
    on_connect = function(client)
      ...
      local main_module = require("codriver.vendor.claudecode")
    ...
    on_disconnect = function(client, code, reason)
      ...
      local diff = package.loaded["codriver.vendor.claudecode.diff"]
      if diff then
