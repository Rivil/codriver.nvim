...
local logger = require("codriver.vendor.claudecode.logger")
...
M.state = {
  config = require("codriver.vendor.claudecode.config").defaults,
  server = nil,
  port = nil,
  auth_token = nil,
  initialized = false,
...
function M.is_claude_connected()
  ...
  local server_module = require("codriver.vendor.claudecode.server.init")
...
function M._ensure_terminal_visible_if_connected()
  ...
  local terminal = require("codriver.vendor.claudecode.terminal")
...
function M.setup(opts)
  ...
  local config = require("codriver.vendor.claudecode.config")
  ...
  local terminal_setup_ok, terminal_module = pcall(require, "codriver.vendor.claudecode.terminal")
  ...
  local diff = require("codriver.vendor.claudecode.diff")
...
function M.start(show_startup_notification)
  ...
  local lockfile = require("codriver.vendor.claudecode.lockfile")
...
function M._create_commands()
  ...
  local function handle_send_normal(opts)
    ...
    if is_tree_buffer then
      local integrations = require("codriver.vendor.claudecode.integrations")
    ...
    local selection_module_ok, selection_module = pcall(require, "codriver.vendor.claudecode.selection")
  ...
  local function handle_send_visual(visual_data, opts)
    ...
    if is_tree_buffer then
      local integrations = require("codriver.vendor.claudecode.integrations")
      local visual_cmd_module = require("codriver.vendor.claudecode.visual_commands")
      local files, error
    ...
    if visual_data then
      local visual_commands = require("codriver.vendor.claudecode.visual_commands")
  ...
  vim.api.nvim_create_user_command("ClaudeCodeAdd", function(opts)
    ...
    if file_path:match("^[%%#<]") then
      ...
      file_path = require("codriver.vendor.claudecode.utils").expand_tilde(file_path)
    end
  ...
  local terminal_ok, terminal = pcall(require, "codriver.vendor.claudecode.terminal")
  if terminal_ok then
