...
local claudecode_server_module = require("codriver.vendor.claudecode.server.init")
...
local function load_provider(provider_name)
  if not providers[provider_name] then
    local ok, provider = pcall(require, "codriver.vendor.claudecode.terminal." .. provider_name)
    if ok then
...
local function build_config(opts_override)
  ...
  if not resolved_cwd and effective_config.git_repo_cwd then
    local ok_r, cwd_mod = pcall(require, "codriver.vendor.claudecode.cwd")
...
function M.setup(user_term_config, p_terminal_cmd, p_env)
  ...
  require("codriver.vendor.claudecode.terminal.paste_fix").apply(defaults.fix_streamed_paste)
end
